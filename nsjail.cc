	...
	while (poll(fds.data(), fds.size(), -1) > 0) {
		...
		for (size_t i = 0; i < fds.size() - 1; i += 2) {
			...
			if (read_ready && write_ready) {
				if (splice(nsjconf->pipes[i / 2].first, nullptr,
					nsjconf->pipes[i / 2].second, nullptr, 4096,
					SPLICE_F_NONBLOCK) == -1) {
					PLOG_E("splice");
				}
				fds[i].events = POLLIN;
			...
			if ((fds[i].revents & (POLLHUP | POLLERR)) != 0 ||
				...
				cleanup = true;
				close(nsjconf->pipes[i / 2].first);
				close(nsjconf->pipes[i / 2].second);
				nsjconf->pipes[i / 2].first = 0;
				nsjconf->pipes[i / 2].second = 0;
			}
		}
		if (cleanup) {
			break;
